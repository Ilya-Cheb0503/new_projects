...
def create_spreadsheet(service):
    ...
    # Сформирируйте запрос и выполните его
    request = service.spreadsheets().create(body=spreadsheet_body)
    response = request.execute()
    spreadsheet_id = response['spreadsheetId']

    # Верните ID созданного документа
    return spreadsheet_id


def set_user_permissions(spreadsheetId, credentials):
    # Место для вашего кода
    spreadsheet_body = {'type': 'user',  # Тип учётных данных.
                        'role': 'writer',  # Права доступа для учётной записи.
                        'emailAddressyy': 'ya_hungry_chef@gmail.com'}  # Ваш личный гугл-аккаунт.
